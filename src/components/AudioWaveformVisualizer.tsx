...
export const AudioWaveformVisualizer: React.FC<AudioWaveformVisualizerProps> = ({
  ...
  if (!isPlaying) {
    return (
      <button
        ...
        className="group relative inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-neutral-900 hover:bg-neutral-800 dark:bg-neutral-100 dark:hover:bg-white text-white dark:text-neutral-900 text-xs font-semibold shadow-sm transition-all duration-200 active:scale-95"
        title="Play synthesized ElevenLabs tactical voice briefing"
      >
        <span className="w-5 h-5 rounded-full bg-white/20 dark:bg-neutral-900/10 flex items-center justify-center">
          <Play className="w-3 h-3 fill-current text-white dark:text-neutral-900 translate-x-0.5" />
        </span>
        <span className="tracking-tight">Play AI Audio Briefing</span>
        <span className="px-1.5 py-0.5 rounded text-[10px] font-mono font-bold bg-purple-500/30 text-purple-200 dark:text-purple-700">
          ElevenLabs
        </span>
      </button>
    );
  ...
  return (
    <div 
      ...
      className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-neutral-900 text-white border border-neutral-800 shadow-md transition-all duration-300 animate-in fade-in"
    >
